...
if [[ ! $x265 = "n" ]]; then
    ...
    if [[ $compile = "true" ]]; then
        ...
        rm -f $LOCALDESTDIR/bin-video/libx265_main.dll
        ...
        # shared 8-bit libx265_main.dll
        cmake -G "MSYS Makefiles" $xpsupport ../../source -DHG_EXECUTABLE=/usr/bin/hg.bat -DENABLE_CLI:BOOLEAN=OFF -DCMAKE_CXX_FLAGS_RELEASE:STRING="-O3 -DNDEBUG $CXXFLAGS" -DCMAKE_CXX_FLAGS="-static-libgcc -static-libstdc++" -DCMAKE_C_FLAGS="-static-libgcc -static-libstdc++"

        make -j $cpuCount
        cp libx265.dll $LOCALDESTDIR/bin-video/libx265_main.dll

        rm -rf $LOCALBUILDDIR/x265-hg/build/msys/*

        if [[ $x265 = "y" ]]; then
            # 16-bit static x265.exe
            cmake -G "MSYS Makefiles" $xpsupport $assembly -DHIGH_BIT_DEPTH=1 -DCMAKE_INSTALL_PREFIX:PATH=$LOCALDESTDIR -DENABLE_CLI:BOOLEAN=ON  ../../source -DENABLE_SHARED:BOOLEAN=OFF -DHG_EXECUTABLE=/usr/bin/hg.bat -DCMAKE_CXX_FLAGS="-static-libgcc -static-libstdc++" -DCMAKE_C_FLAGS="-static-libgcc -static-libstdc++" -DBIN_INSTALL_DIR=$LOCALDESTDIR/bin-video -DCMAKE_CXX_FLAGS_RELEASE:STRING="-O3 -DNDEBUG $CXXFLAGS" -DCMAKE_EXE_LINKER_FLAGS_RELEASE:STRING="$LDFLAGS -static"
        else
            # 16-bit static libx265.a
            cmake -G "MSYS Makefiles" $xpsupport $assembly -DHIGH_BIT_DEPTH=1 -DCMAKE_INSTALL_PREFIX:PATH=$LOCALDESTDIR -DENABLE_CLI:BOOLEAN=OFF ../../source -DENABLE_SHARED:BOOLEAN=OFF -DHG_EXECUTABLE=/usr/bin/hg.bat -DCMAKE_CXX_FLAGS="-static-libgcc -static-libstdc++" -DCMAKE_C_FLAGS="-static-libgcc -static-libstdc++"
        fi
