...
if do_pkgConfig "gnutls = $gnutls_ver"; then
    ...
    log "configure" ../configure --prefix=$LOCALDESTDIR --disable-shared --build=$MINGW_CHOST --disable-cxx \
...
if [[ ! $x264 = "n" ]]; then
    ...
    if [[ $compile = "true" ]] || [[ $x264 != "l" && ! -f "$LOCALDESTDIR/bin-video/x264.exe" ]]; then
        ...
        if [[ $x264 != "l" ]]; then
            ...
            CFLAGS="$(echo $CFLAGS | sed 's/ -O2 / /')" do_configure --bit-depth=10 $extracommands
            do_make
        ...
        CFLAGS="$(echo $CFLAGS | sed 's/ -O2 / /')" do_configure --bit-depth=8 $extracommands
        do_makeinstall
...
if [[ $ffmpeg != "n" ]]; then
    ...
    if [[ $compile = "true" ]] || [[ $buildFFmpeg = "true" && $ffmpegUpdate = "y" ]] ||
        ...
        do_checkForOptions "--enable-gcrypt" &&
            do_patch "ffmpeg-0003-rtmpdh-Initialize-gcrypt-before-using-it.patch" am &&
            do_patch "ffmpeg-0004-configure-Use-libgcrypt-config-if-available.patch" am
...
if [[ $xpcomp = "n" && $mpv = "y" ]] && pkg-config --exists "libavcodec libavutil libavformat libswscale"; then
    ...
    if [[ $compile = "true" ]] || [[ $newFfmpeg = "yes" ]]; then
        ...
        [[ ! -f waf ]] && $python bootstrap.py >/dev/null 2>&1
        if [[ -d build ]]; then
            $python waf distclean >/dev/null 2>&1
