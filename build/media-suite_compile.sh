...
if [[ $vlc = y ]]; then
    ...
    if do_vcs "$SOURCE_REPO_QTBASE"; then
        ...
        sed -i 's;message(FATAL_ERROR \"The;message(\"The;' \
            "$MINGW_PREFIX"/lib/cmake/zstd/zstdTargets.cmake

        local _platform=win32-g++
        [[ ${CC/ccache /} = clang ]] && _platform=win32-clang-g++
        QTBase_config=(
            -static
            -opensource
            -confirm-license
            -nomake examples
            -nomake tests
            -no-{dbus,gif,openssl,pch,pkg-config}
            -no-feature-{androiddeployqt,concurrent,freetype,network,pdf,printsupport,sql,style-fusion,testlib,xml,zstd}
            -no-feature-{dockwidget,keysequenceedit,lcdnumber,mdiarea,movie,pkg-config,splashscreen,statusbar,statustip,syntaxhighlighter,undoview,whatsthis}
            -system-{harfbuzz,libjpeg,libpng,zlib}
            -prefix "$(cygpath -pm $LOCALDESTDIR)"
            -datadir "$(cygpath -pm $LOCALDESTDIR/share/qt6)"
            -archdatadir "$(cygpath -pm $LOCALDESTDIR/share/qt6)"
        )
        ...
        create_build_dir
        CXXFLAGS+=" -DPCRE2_STATIC" config_path=.. log "configure" ../configure \
            "${QTBase_config[@]}" -- -G Ninja -DCMAKE_BUILD_TYPE=Release \
            -DCMAKE_TOOLCHAIN_FILE="$(cygpath -pm $LOCALDESTDIR/etc/toolchain.cmake)" \
            "${QTBase_cmake_config[@]}"
        do_ninja && do_ninjainstall
        do_checkIfExist

        cat >> "$LOCALDESTDIR/share/qt6/mkspecs/$_platform/qmake.conf" <<'EOF'
CONFIG += static
EOF
    ...
    if do_vcs "$SOURCE_REPO_LIBTHAI"; then
    ...
    if flavor=vlc do_vcs "$ffmpegPath#branch=release/6.1"; then
        ...
        [[ -f config.mak ]] && log "distclean" make distclean
        mapfile -t vlc_ffmpeg_opts < <(
            printf '%s\n' "--disable-encoders" "--disable-muxers"
        )
        create_build_dir vlc
        config_path=.. do_configure "${FFMPEG_BASE_OPTS[@]}" \
            --prefix="$LOCALDESTDIR/opt/vlcffmpeg" \
            --disable-decoder=opus --disable-encoder=vorbis --disable-{programs,devices,filters,decoders,hwaccels,encoders,muxers} \
            --disable-{autodetect,avdevice,avfilter,swresample,debug,doc,network} \
            --disable-bsfs --enable-bsf=vp9_superframe --disable-protocol=concat \
    ...
    if do_vcs "$SOURCE_REPO_VLC"; then
		...
		sed -e 's;"-qtconf", qtconf, ;;' \
            -e 's;"qmlimportscanner";"qmlimportscanner.exe";' \
