...
_zlib_uninstall=(lib/cmake/{zlib,minizip} z{conf,lib}.h include/minizip libminizip.{,l}a minizip.pc)
if [[ $zlib = n ]]; then
    ...
    if [[ $zlib = y ]]; then
        ...
        if do_vcs "$SOURCE_REPO_ZLIBCHROMIUM"; then
            ...
            do_cmakeinstall -DINSTALL_PKGCONFIG_DIR="${LOCALDESTDIR}/lib/pkgconfig" -DENABLE_SIMD_OPTIMIZATIONS=ON \
