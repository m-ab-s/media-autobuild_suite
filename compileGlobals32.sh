...
if [ -f "pcre-8.33/compile.done" ]; then
	...
	echo ----------------------------------
		...
		cd pcre-8.33
		./configure --prefix=$LOCALDESTDIR --enable-pcre16 --enable-pcre32 --enable-unicode-properties --enable-newline-is-any --enable-shared=no --enable-static=yes
		make -j $cpuCount
		make install
		echo 'finish' > compile.done
...
if [ -f "rtmpdump-2.3/compile.done" ]; then
	...
	cd rtmpdump-master
	sed -i 's/LIB_OPENSSL=-lssl -lcrypto $(LIBZ)/LIB_OPENSSL=-lssl -lcrypto $(LIBZ) -L\/local32\/lib/g' Makefile
	sed -i 's/LIB_OPENSSL=-lssl -lcrypto $(LIBZ)/LIB_OPENSSL=-lssl -lcrypto $(LIBZ) -L\/local32\/lib/g' librtmp/Makefile
