...
if [ -f "$LOCALDESTDIR/bin/fftwf-wisdom.exe" ]; then
    ...
    echo -------------------------------------------------
		...
		echo -ne "\033]0;compile fftw 64Bit\007"
...
if [ -f "$LOCALDESTDIR/bin/fluid.exe" ]; then
    ...
    echo -------------------------------------------------
		...
		echo -ne "\033]0;compile fltk 64Bit\007"
...
if [ -f "$LOCALDESTDIR/bin/exrdisplay.exe" ]; then
    ...
    echo -------------------------------------------------
		...
		echo -ne "\033]0;compile IlmBase 64Bit\007"
		...
		echo -ne "\033]0;compile OpenEXR 64Bit\007"
		cd OpenEXR
		./bootstrap
		...
		echo -ne "\033]0;compile OpenEXR_Viewers 64Bit\007"
		cd OpenEXR_Viewers
		./bootstrap 
...
if [ -f "$LOCALDESTDIR/bin/exiv2.exe" ]; then
    ...
    echo -------------------------------------------------
		...
		echo -ne "\033]0;compile exiv2 64Bit\007"
...
set(ZLIB_LIBRARY       /local64/lib/libz.a)
set(ZLIB_INCLUDE_DIR   /local64/include)
EOF
